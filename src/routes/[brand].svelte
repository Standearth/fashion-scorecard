<script context="module">
	...
	export async function load({ params, fetch }) {
		...
		const content = await fetch(`${base}/${slug}.json`).then((r) => r.json());
		return {
			props: content
		};
	}
	...
	function processSocialClick(channel) {
		...
		var twitterShare = {
			message: encodeURIComponent(
				socialContent.Tweet + ' https://fashion.stand.earth/' + content.path
			)
		};

		var emailShare = {
			subject: encodeURI(
				content.brand + ' scored ' + content.grade + ' in our fossil-free fashion scorecard'
			),
			message:
				encodeURI(socialContent.Email) +
				'%0A%0A' +
				encodeURI('https://fashion.stand.earth/' + content.path)
		};
		var facebookShare = {
			url:
				encodeURIComponent('https://fashion.stand.earth/' + content.path) +
...
<svelte:head>
	...
	<meta
		name="title"
		content="{content.brand} scored {content.grade} in our Fossil-free Fashion Scorecard | Stand.earth"
	/>
	<meta
		property="og:title"
		content="{content.brand} scored {content.grade} in our Fossil-free Fashion Scorecard | Stand.earth"
								...
								<Row>
									<Col sm="1" lg="1">
										<div class="section-icon">
											<img
												alt="Climate commitments & supply chain energy transparency"
							...
							<div class="acc-header" slot="header">
								<Row>
									<Col sm="1" lg="1">
										<div class="section-icon">
											<img
												alt="Renewable & energy-efficient manufacturing"
					...
					<div class="brand-share">
						<p>
							...
							<a
								rel="external"
								target="_new"
								href="mailto:?subject={processSocialClick('Email')
									.subject}&body={processSocialClick('Email').message}"
								><Fa icon={faEnvelope} color="#2F5E80" size="2x" /></a
							>
						</p>
					</div>
				</Col>
			...
			<Row>
				<Col sm="12" lg={{ size: 8, offset: 2 }}>
					<div class="similar-companies">
						...
						<p>
							<a target="_self" href="/brand-scores"
								><Fa icon={faArrowLeft} size="2x" /> View all companies</a
							>
						</p>
						<hr />
					</div>
